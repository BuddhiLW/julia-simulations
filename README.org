...
** 1.6
#+begin_quote
1. Suppose n people sit down at random and independently of each other in an
...
$\dfrac{n!}{m!}$ possible arrangements possible, for this =m=-sized group of sits
of interest, which will be of *no concern*. Because, the possible ways to chose
=C(n+k,n)= doesn't consider order.

Still, we can still have =C(n-m+k, n-m)= possible fills of left-over placements.
